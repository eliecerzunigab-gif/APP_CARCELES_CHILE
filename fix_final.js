...
// The complete JS that should follow
const completeJS = `
...
function renderizarAlertas(){
  ...
  var count=document.getElementById('alertas-count');
  var filtered=STATE.alertas.filter(function(a){return !a.resuelta});
  if(STATE.recintoActivo)filtered=filtered.filter(function(a){return a.recinto_id===STATE.recintoActivo.id});
  count.textContent=filtered.length;
  if(filtered.length===0){list.innerHTML='<div class="loading">Sin alertas activas